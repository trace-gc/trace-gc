...
- **Benchmark CLI Subcommand (`trace-gc benchmark`)**: Added a CLI subcommand to reproduce comparative benchmark token counts and probe results locally against bundled fixtures (`--sample`) or custom trace files (`trace-gc benchmark <path>`).
