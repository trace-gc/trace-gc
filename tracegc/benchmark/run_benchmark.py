...
Maintains tracegc/benchmark/checkpoint.json. Runs free local methods to
...
from tracegc.benchmark.scoring import (
...
def run_benchmark(simulate: bool = False):
    ...
    for filename in filenames:
        ...
        # TraceGC Pipeline
        if not is_completed(filename, "tracegc_pipeline", "n/a", 1):
            def run_pipeline():
            ...
            save_checkpoint(completed)
            print(f"Completed local tracegc_pipeline for {filename}", flush=True)
